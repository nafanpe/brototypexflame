-- Fix complaint number generation to use continuous sequence
...
BEGIN
  ...
  -- Get the next sequence number across ALL complaints (not just today's)
  SELECT COALESCE(MAX(
    CASE 
  ...
  -- Removed the WHERE clause that filtered by date
